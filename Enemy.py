        ...
        if self.shootCooldown == 0:
            ...
            if self.direction[0] > 1 or self.direction[0] < -1:
                bullet = Bullets(self.enemy_position[0],
                                 self.enemy_position[1], 1, 1, self.speedBullet, self.surface_size,
                                 self.player.player_position[1], self.player.player_position[0])
                ...
                if self.direction[1] < 1:
                    bullet = Bullets(self.enemy_position[0],
                                     ...
                                     self.speedBullet, self.surface_size, self.player.player_position[1],
                                     self.player.player_position[0])
                    ...
                    bullet = Bullets(self.enemy_position[0],
                                     self.enemy_position[1] + (0.1 * self.rect.size[0] * self.direction[0]), 1,
                                     self.speedBullet, self.surface_size, self.player.player_postiion[1],
                                     self.player.player_position[0])
